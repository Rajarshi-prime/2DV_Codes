...
einit = 5 ## Amplitude of psi0s


kinit= 10 ## Intial energy will be distributed among these regions equally in all wave numbers but will have a phase. 
## ----------------------------------------------
...
def evolve_and_save(f,t,x0):
    ...
    for i,ti in enumerate(t[:-1]):
        ...
        if i%st ==0:
            ...
            savePath = curr_path/f"data/TG/N_{Nx}_Re_{np.round(1/nu,2)},dt_{dt}/time_{t[i]}"
            savePath.mkdir(parents=True, exist_ok=True)
            np.save(f"data/TG/N_{Nx}_Re_{np.round(1/nu,2)},dt_{dt}/time_{t[i]}/w", x_old)
    ...
    savePath = curr_path/f"data/TG/N_{Nx}_Re_{np.round(1/nu,2)},dt_{dt}/time_{t[i+1]}"
    savePath.mkdir(parents=True, exist_ok=True)
    np.save(f"data/TG/N_{Nx}_Re_{np.round(1/nu,2)},dt_{dt}/time_{t[i+1]}/w", x_old)
    return etot
## ---------------------------------------------------------   
...
xi0r = np.sin(x)*np.sin(y/2)
xi0 = fft.rfft2(xi0r)
psi0 = - ifft2(np.where(lap ==0 ,np.inf, 1/lap)*xi0)
...
np.save(f"data/TG/N_{Nx}_Re_{np.round(1/nu,2)},dt_{dt}/parameters",param)
# file.close()
